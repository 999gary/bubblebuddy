...
static memory_arena nuklear_memory;
...
nk_d3d9_render(enum nk_anti_aliasing AA)
{
    ...
    /* convert from command queue into draw list and draw to screen */
    {
        ...
        memory_arena_state state = memory_arena_begin_scratch_memory(&nuklear_memory);
        /* convert shapes into vertexes */
        nk_buffer_init(&vbuf, &alloc, NK_DEFAULT_COMMAND_BUFFER_SIZE);
        nk_buffer_init(&ebuf, &alloc, NK_DEFAULT_COMMAND_BUFFER_SIZE);
        ...
        memory_arena_end_scratch_memory(&nuklear_memory, state);
    }
...
nk_d3d9_clipboard_paste(nk_handle usr, struct nk_text_edit *edit)
{
    ...
    if (utf8size) {
        memory_arena_state state = memory_arena_begin_scratch_memory(&nuklear_memory);
        char *utf8 = (char *)memory_arena_alloc(&nuklear_memory, utf8size);
        if (utf8) {
...
nk_d3d9_init(IDirect3DDevice9 *device, int width, int height)
{
    ...
    nk_buffer_init(&d3d9.cmds, &alloc, NK_BUFFER_DEFAULT_INITIAL_SIZE);
...
nk_d3d9_font_stash_begin(struct nk_font_atlas **atlas)
{
    ...
    nk_font_atlas_init_custom(&d3d9.atlas, &alloc, &alloc);
